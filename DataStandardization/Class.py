            ...
            for filename in os.listdir(covariate_dir):
                if filename.endswith(".tsv"):
                    ...
                    for val in end_points:
                        ...
                        final_series.to_csv(path_or_buf=f"{Class_dir}{val}.txt", sep="\t", header=True, na_rep="NA")

                    for var in variables:
                        if var.startswith(end_points):
                            continue
                        else:
                            df[var].to_csv(path_or_buf=f"{covariate_dir}{_}{var}.txt", sep="\t", header=True, na_rep='NA')
